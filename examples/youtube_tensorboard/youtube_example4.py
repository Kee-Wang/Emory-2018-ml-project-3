...
def conv_layer(input, channels_in, channels_out, name='conv'):
  with tf.name_scope(name):
    w = tf.Variable(tf.truncated_normal([5, 5, channels_in, channels_out], stddev=0.1), name='Weight')
...
def fc_layer(input, channels_in, channels_out, name='fc'):
  with tf.name_scope(name):
    w = tf.Variable(tf.truncated_normal([channels_in, channels_out], stddev=0.1), name='Weight')
    b = tf.Variable(tf.constant(0.1, shape=[channels_out]), name='Bias')
...
writer = tf.summary.FileWriter("../../tensorboard/youtube_example/4")
