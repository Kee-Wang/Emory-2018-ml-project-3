...
# Create the network
conv1 = conv_layer(x_image, 1, 32) #Channels_out = 32 meaning we want 32 filters
pool1 = tf.nn.max_pool(conv1, ksize=[1,2,2,1], strides=[1,2,2,1], padding="SAME")
#Only shrinks height and length but not DEPTH, here he depth (channel) is still 32

conv2 = conv_layer(pool1, 32, 64) #Meaning input 32 filters
...
fc1 = fc_layer(flattened, 7*7*64, 1024) #Each image 7*7*64 are converted to a 1024 node.
logits = fc_layer(fc1, 1024, 10) # Convert 1024 node to 10 nodes
